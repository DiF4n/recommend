...
select_dict = sorted(select_dict.items(), key=lambda item: item[1], reverse=False)
...
def dataPreprocess(line):
    ...
    print()
    fields = tf.io.decode_csv(line, record_defaults=defs, select_cols = select_cols, field_delim='\t', na_value = '\\N')
    features = dict(zip(feature_list,fields))
    label = {'pred_addf': features.pop('addf_num'), 'pred_suc': features.pop('suc_num')}
...
def feature_process():
    ...
    for con_col in base_param.NUMERIC_FEATURE_KEYS:
        dnn_feature_columns[con_col] = tf.feature_column.numeric_column(con_col)
        inputs[con_col] = layers.Input(name=con_col, shape=(), dtype='float32')

    for con_col in base_param.SUCCESS_FEATURE_KEYS:
        successScore_feature_columns[con_col] = tf.feature_column.numeric_column(con_col)
        inputs[con_col] = layers.Input(name=con_col, shape=(), dtype='float32')
    
    for con_col in base_param.ADDF_FEATURE_KEYS:
        if con_col in ['avg_addf_score']:
            addfScore_feature_columns[con_col] = tf.feature_column.bucketized_column(tf.feature_column.numeric_column(con_col), boundaries = base_param.BUCKET_DICT[con_col])
        else:
            addfScore_feature_columns[con_col] = tf.feature_column.numeric_column(con_col)
            if con_col in base_param.BUCKET_FEATURE_KEYS:
                addfScore_feature_columns[con_col+'_bucket'] = tf.feature_column.bucketized_column(tf.feature_column.numeric_column(con_col), boundaries = base_param.BUCKET_DICT[con_col])
    ...
    for hash_col in base_param.HASH_FEATURE_KEYS:
        dnn_feature_columns[hash_col] = tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_hash_bucket(hash_col,base_param.HASH_NUM[hash_col]),32)
        inputs[hash_col] = layers.Input(name=hash_col, shape=(), dtype='string') 
...
def addf_wide_and_deep_Regression(inputs, addfScore_feature_columns, dnn_feature_columns, dnn_hidden_units):
    ...
    model.compile(optimizer='rmsprop',loss='mse', metrics=[tf.keras.metrics.RootMeanSquaredError(), 'mae', custom_mse])
    return model
...
def train(train_begin_date,train_end_date,test_date):
    ...
    trainFilePaths = [FilePath + f for f in date_list if isfile(join(FilePath, f))]
    ...
    model = addf_wide_and_deep_Regression(inputs, addfScore_feature_columns.values(), dnn_feature_columns.values(), DNN_HIDDEN_UNITS)

    if os.path.exists(checkpoint_cb):
        print("load model")
        model = models.load_model(checkpoint_cb, custom_objects = {'custom_mse':custom_mse})
    
    checkpoint_cb = callbacks.ModelCheckpoint(checkpoint_cb, monitor='loss', verbose=1, save_best_only=True, mode='min')

    printlog("step4: train model with current checkpoint...")
    model.fit(train_ds, validation_data=val_ds, epochs=10, callbacks=[checkpoint_cb])
